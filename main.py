...
def run_cloud_service(local_folder: str, client: CloudService) -> None:
    ...
    local_files = {
        file_name: datetime.datetime.fromtimestamp(
            os.path.getmtime(os.path.join(local_folder, file_name))
        )
    ...
    if common_files:
        for file_name in common_files:
            if (
                local_files[file_name].timestamp()
                > (
                    datetime.datetime.fromisoformat(
                        cloud_files[file_name].replace("Z", "+00:00")
                    )
                ).timestamp()
            ):
